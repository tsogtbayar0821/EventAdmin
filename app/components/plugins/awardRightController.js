angular
    ...
    .controller('awardRightCtrl', [
        ...
        '$http',
        'variables',
        function ($scope,$timeout,$compile,variables,ts_data,$http,variables) {

            $scope.table_data = [];
            $scope.getPhotographers = function(){
                //api/v1/photographers
               var res = $http.get(variables.url + 'api/v1/exceptForAdmin');
                res.success(function(data, status, headers, config) {
                   if(data['success'] == true){
                       $scope.table_data = data['result'];
                       for (var i = 0; i < $scope.table_data.length; i++)
                            $scope.table_data[i].radio = $scope.table_data[i].awardRight == true ? "On" : "Off";
                   }else{
            ...
            $scope.awardRightClick = function(item){
                item.awardRight = item.radio == "On" ? true : false;
              var dataObj = {
                    'email': item.email,
                    ...
                    'awardRight': item.awardRight,
                    'startDateTime': item.startDateTime
                };

                var res = $http.post(variables.url + 'api/v1/photographers/' + item.id, dataObj);
                res.success(function(data, status, headers, config) {
                   if(data['success'] == true){
                       var kk = 0;//UIkit.modal.alert("It was successful to save event");
                   }else{
