angular
    ...
    .controller('userListCtrl', [
        ...
        'ts_data',
        '$http',
        function ($scope,$timeout,$compile,variables,ts_data, $http) {
            ...
            $scope.getUsers = function(){
              ...
              var dataObj = {
                ...
                var res = $http.post(variables.url + 'api/v1/admincontroller/commentProhibition/users', dataObj);
                res.success(function(data, status, headers, config) {
                   if(data['success'] == true){
                        $scope.table_data = data['result'];   
                        for (var i = 0; i < $scope.table_data.length; i++)                 
                        {
                            $scope.table_data[i].radio = $scope.table_data[i].commentProhibition == true ? 'On' : 'Off';
                        }
                   }else{
            ...
            $scope.getUsers();
            $scope.prohibitionClick = function(item){
                item.commentProhibition = item.radio == 'On' ? true : false;
                var dataObj = {
                    'commentProhibition': item.commentProhibition
                };
                var res = $http.post(variables.url + 'api/v1/admincontroller/saveCommentProhibition/' + item.id, dataObj);
                res.success(function(data, status, headers, config) {
                   if(data['success'] == true){
                        var kk = 0;
                   }else{
            ...
            // for (var i = 0; i < $scope.table_data.length; i++)
            // {
            //     $scope.table_data[i].avatar = "./assets/img/gallery/Image16@2x.jpg";
            // }
