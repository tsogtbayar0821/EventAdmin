    ...
    .controller('photoStatisticCtrl', [
        ...
        function ($scope,$interval,$window,mg_chart_linked_1,mg_chart_linked_2,mg_confidence_band,mg_currency) {
            ...
            // metrics graphics
               ...
               // confidence band
                $scope.mg_comment_band_data = [mg_confidence_band];

                // for (var i = 0; i < $scope.mg_comment_band_data.length; i++) {
                //     $scope.mg_comment_band_data[i] = MG.convert.date($scope.mg_comment_band_data[i], 'date');
                // }

                $scope.mg_comment_band_options = {
                    title: "Comments",
                    description: "This is an example of a graphic with a comment band and extended x-axis ticks enabled.",
                    data: $scope.mg_comment_band_data,
                    format: 'percentage',
                    show_secondary_x_label: false,
                    show_comment_band: ['l', 'u'],
                    x_accessor: 'date',
                    y_accessor: 'value',
                    x_extended_ticks: true
                };
            ...
            if ( $(c3chart_spline_id).length ) {
                ...
                var c3chart_spline = c3.generate({
                    ...
                    data: {
                        columns: [
                            ['Views', 30, 200, 100, 400, 150, 250],
                            ['Downloads', 130, 100, 140, 200, 150, 50],
                            ['Comments', 500, 75, 250, 30, 40, 90]
                        ],
                        type: 'spline'
                    },
                    color: {
            ...
            if ( $(c3chart_area_stacked_id).length ) {
                ...
                var c3chart_area_stacked = c3.generate({
                    ...
                    data: {
                        columns: [
                            ['Views', 300, 350, 300, 0, 0, 120],
                            ['Downloads', 130, 100, 140, 200, 150, 50],
                            ['Comments', 170, 130, 30, 100, 70, 20]
                        ],
                        types: {
                            data1: 'area-spline',
                            data2: 'area-spline'
